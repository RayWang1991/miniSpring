...
public class StaticValueRefResolver {
    ...
    public ValueOrRef resolveRefValueForProperty(Element element) {
        if (element.hasAttribute(XmlBeanDefinitionProvider.ATTRIBUTE_VALUE)) {
            return DefaultValueOrRef.plainValue(null, element.getAttribute(XmlBeanDefinitionProvider.ATTRIBUTE_VALUE));
        } else if (element.hasAttribute(XmlBeanDefinitionProvider.ATTRIBUTE_VALUE_REF)) {
            return DefaultValueOrRef.ref(null, element.getAttribute(XmlBeanDefinitionProvider.ATTRIBUTE_VALUE_REF), true);
        } else {
            NodeList nodeList = element.getChildNodes();
            if (nodeList == null || nodeList.getLength() == 0) {
                return null;
    ...
    public ValueOrRef resolveElementByName(String name, Element element) {
        if (name.equals(XmlBeanDefinitionProvider.ELEMENT_REF)) {
            return readRef(element);
        } else if (name.equals(XmlBeanDefinitionProvider.ELEMENT_VALUE)) {
            return readValue(element);
        } else if (name.equals(XmlBeanDefinitionProvider.ELEMENT_NULL)) {
            return readNull();
        } else if (name.equals(XmlBeanDefinitionProvider.ELEMENT_SET)) {
            return readSet(element);
        } else if (name.equals(XmlBeanDefinitionProvider.ELEMENT_LIST)) {
            return readList(element);
        } else if (name.equals(XmlBeanDefinitionProvider.ELEMENT_BEAN)) {
    ...
    public ValueOrRef readList(Element element) {
        ...
        // type of elements
        Class<?> clazz = TypeUtils.typeForName(element.getAttribute(XmlBeanDefinitionProvider.ATTRIBUTE_TYPE));
